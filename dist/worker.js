...
import { proactiveTokenRefresh, projectsFetch } from "./lib/zoho-client.js";
...
async function getAuthState(ctx) {
    ...
    const services = (await ctx.state.get({ scopeKind: "instance", stateKey: "bridge.services" })) ?? [];
    ...
    async setup(ctx) {
        ...
        ctx.data.register("zoho-portals", async () => {
            try {
                ...
                if (!result.ok)
                    return { portals: [], error: `API error: ${result.status}`, rawResponse: result.data };
                const data = result.data;
                const portals = (data.portals ?? []);
                return { portals: portals.map((p) => ({ id: String(p.id ?? p.id_string ?? ""), name: String(p.name ?? "") })) };
            }
            catch (e) {
                return { portals: [], error: String(e) };
            }
        });
        ctx.data.register("zoho-projects-list", async (params) => {
            try {
                const config = (await ctx.config.get());
                const portalId = params.portalId || config.portalId;
                if (!portalId)
                    return { projects: [], error: "No portalId" };
                const result = await projectsFetch(ctx, "GET", `/portal/${portalId}/projects/?range=100&status=active`);
                if (!result.ok)
                    return { projects: [], error: `API error: ${result.status}` };
                const data = result.data;
                ...
                return {
                    projects: projects.map((p) => ({
                        ...
                        group: p.group ? { id: String(p.group.id ?? ""), name: String(p.group.name ?? "") } : null,
                        clientCompany: p.company ? { id: String(p.company.id ?? ""), name: String(p.company.name ?? "") } : null,
                    })),
                };
            }
            catch (e) {
        ...
        ctx.data.register("zoho-users-list", async (params) => {
            try {
                ...
                const portalId = params.portalId || config.portalId;
                if (!portalId)
                    return { users: [], error: "No portalId" };
                const result = await projectsFetch(ctx, "GET", `/portal/${portalId}/users/?range=200&status=active`);
                if (!result.ok)
                    return { users: [], error: `API error: ${result.status}`, rawResponse: result.data };
                const data = result.data;
                ...
                return {
                    users: users.map((u) => ({
                        ...
                        role: String(u.role ?? ""),
                        company: u.company ? { id: String(u.company.id ?? ""), name: String(u.company.name ?? "") } : null,
                    })),
                };
            }
            catch (e) {
        ...
        ctx.data.register("paperclip-agents-by-company", async (params) => {
            try {
                const companyIds = (params.companyIds ?? "").split(",").filter(Boolean);
                const result = {};
                ...
                return { agentsByCompany: {}, error: String(e) };
            }
        });
        ctx.data.register("paperclip-projects-by-company", async (params) => {
            try {
                ...
                return { projectsByCompany: {}, error: String(e) };
            }
        });
        ...
        ctx.data.register("zoho-clients-list", async (params) => {
            try {
                ...
                const portalId = params.portalId || config.portalId;
                if (!portalId)
                    return { clients: [], error: "No portalId" };
                const result = await projectsFetch(ctx, "GET", `/portal/${portalId}/clients/?range=100`);
                if (!result.ok)
                    return { clients: [], error: `API error: ${result.status}`, rawResponse: result.data };
                const data = result.data;
        ...
        ctx.data.register("saved-mappings", async () => {
            const orgMapping = (await ctx.state.get({ scopeKind: "instance", stateKey: "zoho.orgMapping" })) ?? [];
            const agentMapping = (await ctx.state.get({ scopeKind: "instance", stateKey: "zoho.agentMapping" })) ?? [];
            const projectMapping = (await ctx.state.get({ scopeKind: "instance", stateKey: "zoho.projectMapping" })) ?? [];
            const ignoredItems = (await ctx.state.get({ scopeKind: "instance", stateKey: "zoho.ignoredItems" })) ?? [];
        ...
        ctx.actions.register("add-service", async (params) => {
            ...
            services.push({ id: serviceId, type: serviceType, name: params.name ?? serviceType, enabled: true, createdAt: new Date().toISOString() });
        ...
        ctx.actions.register("save-all-mappings", async (params) => {
            ...
            if (params.orgMapping) {
                const orgs = params.orgMapping;
                await ctx.state.set({ scopeKind: "instance", stateKey: "zoho.groupMapping" }, orgs.map((m) => ({ groupName: m.zohoCompany, companyId: m.paperclipCompanyId })));
            }
            return { ok: true };
        });
        ...
        ctx.actions.register("request-agent-hire", async (params) => {
            ...
            const companyId = params.companyId;
            if (!agentName || !companyId)
                return { ok: false, error: "agentName and companyId required" };
            try {
                // Find the CEO agent in the company
                const agents = await ctx.agents.list({ companyId, limit: 200, offset: 0 });
                const ceo = agents.find((a) => a.role === "ceo");
                if (!ceo)
                    return { ok: false, error: "No CEO agent found in this company" };
